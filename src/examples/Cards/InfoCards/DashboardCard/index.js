...
const DashboardCard = ({ color, colorcode, title, count, data1, labels, percentage, icon, labelicon, imgicon, selectedCardCode, isSelected }) => {
  ...
  const chartData = (canvas) => {
    ...
    gradient.addColorStop(0, hexToRgba(colorcode, 0.3)); 
...
// console.log(chartRef)
  return (
    <Card sx={{
      ...
      // height: 200
        // height: 163,
        // border: isSelected ? "3px solid #000" : "1px solid transparent", // Highlight selected card
        backgroundColor: isSelected ? selectedCardCode : "#fff", 
      ...
      <MDBox display="flex" justifyContent="space-between" pt={1} px={2}>
        ...
        <MDBox textAlign="right" lineHeight={1.25}>
          ...
          color="text"
          style={{
            color: isSelected ? colorcode : null
          }}
          >
            {title} {!isSelected ? <CallMadeIcon /> : <SouthEastIcon />}
          </MDTypography>
          <MDTypography 
          variant="h4"
          // variant="h3"
          style={{
            color: isSelected ? color === 'purple' ? "#968cff" : colorcode : null
          }}
          >{count}</MDTypography>
        </MDBox>
      </MDBox>
