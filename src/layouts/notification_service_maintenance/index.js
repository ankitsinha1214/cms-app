...
function Notification_service_maintenance_mgmt() {
  ...
  const column = [
    {
      ...
      filterFn: (rows, id, filterValue) => {
        if (filterValue === "Read") {
          return rows.original.isRead === true;
        } else if (filterValue === "Unread") {
          return rows.original.isRead === false;
        }
  ...
  const handleDelete = (row_data) => {
    ...
    axios({
      method: "delete",
      url: process.env.REACT_APP_BASEURL + "notification-service-and-maintenance/" + row_data._id,
      headers: {
  ...
  const countActive = rows.filter(row => row.isRead === true).length;
  const countInactive = rows.filter(row => row.isRead === false).length;
            ...
            <MDBox mx={2} mt={-3} py={3} px={2} variant="gradient" bgColor="info" borderRadius="lg" coloredShadow="info">
              <Grid container direction="row" justifyContent="space-between" alignItems="center">
                <MDTypography variant="h6" color="white">
                  All Notification Service & Maintenance
                </MDTypography>
                <MDButton
...
export default Notification_service_maintenance_mgmt;