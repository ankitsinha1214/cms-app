...
function PopReset(props) {
    ...
    const createUser = (type, charger_id, reason) => {
        ...
        axios({
            method: "post",
            url: process.env.REACT_APP_BASEURL + "session/reset",
            ...
            .catch((error) => {
                console.log(error);
                enqueueSnackbar("An error occurred while reseting the charger.", { variant: 'error' });
    ...
    const handleSubmit = (event) => {
        ...
        console.log(values)
        if (!values.type || !values.charger_id || !values.reason) return enqueueSnackbar('Please Fill All The Details !!!', { variant: 'error' })
        createUser(values.type, values.charger_id, values.reason);
    };
    const handleClose = () => {
        onClose(false);
    };
                        ...
                        <MDBox p={2} component="form" role="form">
                            ...
                            <MDBox p={1}>
                                <MDInput
                                    ...
                                    multiline
                                    rows={5}
                                    placeholder="Enter Reason for the Reset of Charger.. "
                                    margin="dense"
                                    fullWidth={true}
                                    onChange={handleChange}
                                />
